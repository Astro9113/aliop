...
const ethNodeURI = 'http://192.168.0.51:8545';
...
function _processTransactionReceipt(receipt, callback) {
  ...
  if (meta.topic === log.topics[0]){
    ...
    const timestamp = parseInt(dataDecrypted[0])*1000;
...
function _retrieve( shortLink, cb ) {
  const timestamp = (contractInstance.getChainyTimestamp(shortLink)*1000);
