...
def wl_get_cluster_instance_all():
    ...
    for it in ins_list:
        ...
        str_delete_time = time.strftime('%Y-%m-%d %H:%M', todelete_time)
        myins["delete_at"] = str_delete_time
        myins["delete_time"] = myins["created_time_utc"]+wl_delete_after
        appall.append(myins)
    return appall
...
def list_one_obj(insname):
    ...
    created_time_utc = time.mktime(time.strptime(content["created"].split('.')[0], '%Y-%m-%dT%H:%M:%S'))
    myins["created_time_utc"] = created_time_utc
